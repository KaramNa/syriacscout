<div>
    ...
    @endif
    <div class="relative">
        <div class="flex justify-between items-center mb-1 py-2" dir="ltr">
            <div class="h-10 flex items-center">
                ...
                <div class="w-96 flex rounded-lg shadow-sm">
                    <div class="relative flex-grow focus-within:z-10">
                        ...
                        <input wire:model.debounce.500ms="search" class="w-full pl-10 py-3 text-sm leading-4 block rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 focus:outline-none" placeholder="{{__('البحث في ')}} {{ $this->searchableColumns()->map->label->join(', ') }}" type="text" />
                        ...
                        @foreach($this->columns as $index => $column)
                            ...
                            @elseif($column['type'] === 'checkbox')
                                @unless($column['hidden'])
                                    <div class=" table-cell h-12 w-32 py-4 flex justify-center overflow-hidden align-top px-6 border-b border-gray-200 bg-gray-50 text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider focus:outline-none">
                    ...
                    <div class="table-row divide-x divide-blue-200 bg-blue-100">
                        @foreach($this->columns as $index => $column)
                            @if($column['hidden'])
                                ...
                                @endif
                            @elseif($column['type'] === 'checkbox')
                                <div class="overflow-hidden align-top bg-blue-100 px-6 py-5 border-b border-gray-200 text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider flex h-full flex-col items-center space-y-2 focus:outline-none">
                                    <div>SELECT ALL</div>
                                    <div>
                ...
                <div class="p-2 sm:flex items-center justify-between">
                    ...
                    @if(count($this->results))
                        <div class="my-2 sm:my-0 flex items-center">
                            <select name="perPage" class="rounded form-select block w-full pl-3 pr-10 py-2 text-base leading-6 border border-gray-300 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 sm:text-sm sm:leading-5" wire:model="perPage">
    ...
    @endif
    <span class="hidden text-sm leading-5 text-gray-900 text-center text-right bg-gray-50 bg-gray-100 bg-yellow-100"></span>
</div>
